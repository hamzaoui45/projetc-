...
#include "mainwindowpatient.h" // Inclusion pour MainWindowPatient
#include <QBuffer>
#include <QImage>
#include <QDebug>
...
void log::on_connect_2_clicked()
{
    ...
    if (query.exec() && query.next()) {
        ...
        if (poste == "médecin" || poste == "medecin") {
            ...
            QPushButton *mainWindowPatientButton = msgBox.addButton("MainWindowPatient", QMessageBox::AcceptRole); // Option pour MainWindowPatient
            msgBox.exec();

            if (msgBox.clickedButton() == mainWindowVButton) {
                MainWindowV *mv = new MainWindowV();
                mv->show();
            } else if (msgBox.clickedButton() == mainWindowPatientButton) {
                MainWindowPatient *mp = new MainWindowPatient();
                mp->show();
            }
        }
        else if (poste == "agent") {
            ...
            QPushButton *dialogButton = msgBox.addButton("Dialog", QMessageBox::AcceptRole);
            QPushButton *mainWindowRdvButton = msgBox.addButton("MainWindowRdv", QMessageBox::AcceptRole);
            QPushButton *mainWindowPatientButton = msgBox.addButton("MainWindowPatient", QMessageBox::AcceptRole); // Option pour MainWindowPatient
            msgBox.exec();
