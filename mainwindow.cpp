    ...
    , ui(new Ui::MainWindow)
{
    ...
    // Connect to Arduino
    bool ret = arduino.connectArduino();
    if (ret) {
...
void MainWindow::on_valider_2_clicked()
{
    ...
    QRegularExpression lettre("^[A-Za-zÀ-ÖØ-öø-ÿ]+$");
...
void MainWindow::updateA()
{
    ...
    QByteArray qdata = arduino.readFromArduino();
    ...
    if (message.contains("UPDATE_QUANTITY")) {
        ...
        if (lastUpdateProcessed != 0 && currentTime - lastUpdateProcessed < updateCooldown) {
            qDebug() << "Ignoring update due to cooldown";
            return;
        }
        ...
        try {
            ...
            if (query.exec()) {
                ...
                qDebug() << "Failed to update vaccine quantity. Error:" << query.lastError().text();
            }
        } catch (const std::exception& e) {
