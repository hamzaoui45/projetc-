...
bool employee::exportToPDF(const QString& filePath)
{
    ...
    while (query.next())
    {
        ...
        painter.drawText(col5, i, QString::number(query.value("SALAIRE").toDouble(), 'f', 2)); // Salaire (formaté avec 2 décimales)
        painter.drawText(col6, i, query.value("MDP").toString()); // MDP
        painter.drawText(col7, i, query.value("EMAIL").toString()); // Email
        painter.drawText(col8, i, query.value("DATE_EMB").toDate().toString("dd/MM/yyyy")); // Date Embauche (formatée)

        i += 500; // Décalage vertical pour la ligne suivante

        // Si on dépasse la hauteur de la page, on passe à la page suivante
        if (i > 7000) // Ajustez cette valeur en fonction de la hauteur de la page
        {
            pdfWriter.newPage();
    ...
    painter.drawRect(0, 3000, 12000, i - 3000); // Ajuster la largeur du rectangle
...
QSqlQueryModel* employee::trier(int test)
{
    ...
    {
    case 1:
        query.prepare("SELECT * FROM employés ORDER BY ID_EMP ASC"); // Tri par ID (ascendant)
        break;
    case 2:
        query.prepare("SELECT * FROM employés ORDER BY SALAIRE ASC"); // Tri par salaire (ascendant)
        break;
    case 3:
        query.prepare("SELECT * FROM employés  ORDER BY NOM ASC"); // Tri par nom (ascendant)
        break;
    case 4:
        query.prepare("SELECT * FROM employés  ORDER BY DATE_EMB ASC"); // Tri par date d'embauche (ascendant)
        break;
    default:
...
QGraphicsView* employee::statistiquesPoste() {
    ...
    // Parcours de la QMap sans utiliser 'auto'
